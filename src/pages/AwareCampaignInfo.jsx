...
export default function AwareCampaignInfo({ onNext }) {
  ...
  return (
    <PageLayout onNext={onNext} showFooter={false}>
      ...
      <div className="w-full max-w-4xl mx-auto text-center space-y-1">
        <h1 className="text-[5.5vw] sm:text-[2vw] md:text-[5vw] lg:text-[3vw] xl:text-[2.5vw] leading-snug tracking-tight text-center">
      ...
      <div className="w-full max-w-5xl mx-auto space-y-2 mt-2">
        {items.map((item, idx) => (
          <div key={idx} className="flex items-center space-x-6">
            ...
            {/* label in blue box */}
            <div className="bg-blue-600 text-white font-semibold text-xl px-6 py-3 text-center" style={{minWidth:'130px'}}>
              {item.label}
            </div>

            {/* description inside bordered rectangle */}
            <div className="flex-1 border border-blue-600 text-xl text-gray-800 px-4 py-3">
              {item.description}
            </div>
          </div>
        ))}
      ...
      <div className="w-full mt-2 flex justify-center">
        <div className="bg-pink-600 text-white text-2xl font-bold px-8 py-2">
